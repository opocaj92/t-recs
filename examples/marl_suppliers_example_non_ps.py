...
for r in range(runs):
   ...
   for i in range(env.n_players - 1):
      ...
      partners.append(OnPolicyAgent(PPO("MlpPolicy", env.getDummyEnv(i), learning_rate = learning_rate, gamma= gamma, verbose = 1, device = device)))
      env.add_partner_agent(partners[-1], player_num = i + 1)
   print("Ego Agent")
   ego = PPO("MlpPolicy", env, learning_rate = learning_rate, gamma = gamma, verbose = 1, device = device, tensorboard_log = log_savepath)
   ...
   if (num_suppliers + num_fixed) == num_items and not price_into_observation and not quality_into_observation and not attributes_into_observation:
      ...
      for p in range(num_suppliers):
         ...
         hm.set_xticks(range(0, num_users + 1, 10))
         hm.set_xticklabels(f'{c:.1f}' for c in np.arange(0.0, 1.01, 0.1))
         hm.set_yticks(range(0, num_users + 1, 10))
